...
function ajaxDefaultStart(data) {
    ...
    gsap.set(_wrapper, {yPercent: 100, y: 150});
    ...
    return gsap.timeline({defaults: {ease: Expo.easeInOut}})
        .to(_wrapper, {
            yPercent: 0,
            y: 0,
            duration: 1.5
        }).to(svgTop.find('.path-anim'), {
            attr: {d: dataAttr(svgTop.find('.path-anim'), 'to')},
            overwrite: "none",
            duration: 1.5
        }, 0)
        .to(data.current, 1, {y: -400}, "-=1.2");
...
function ajaxDefaultEnd(data) {
    ...
    return gsap.timeline({defaults: {ease: Expo.easeInOut}}).to(bg, {
        yPercent: -100,
        duration: 1.5
    }).to(svgBottom, {
        attr: {d: dataAttr(svgBottom, 'to')},
        overwrite: "none",
        duration: 1.5
    }, 0)
        .fromTo(data.next, 1, {y: 400}, {y: 0, clearProps: true}, "-=1.3");

}

const ajaxDefault = {start: ajaxDefaultStart, end: ajaxDefaultEnd};
