...
export function dsnCreateElementAjax($e, $target, $letter, $targetLtter, $object = {}) {
    ...
    if ($object.before !== undefined)
        $object.before({container, title: title_move, img: img_move});

    $body.append(container);
    return gsap.to(container, 1, {
        autoAlpha: 1, ease: "power2.out", oncomplete: function () {

            if ($object.after !== undefined)
                $object.after({container, title: title_move, img: img_move});
...
export function dsnTransitionElement() {
    ...
    if (!img.length && !title.length) {
        return gsap.to(container, {autoAlpha: 0, ease: "power2.in"});
    }
    ...
    if (title_move.length) {
        ...
        tl.add(gsap.set(title_move, {top: position.top, left: position.left}), "start");


        tlTitleMove
            .to(title_move, 0.4, {css: title.dsnGridStyleObject()})
            .to(titleMoveChar,
                {
                    y: "0",
                    x: "0",
    ...
    tl.to(container, 0.5, {autoAlpha: 0, ease: "none"});
    tl.fromTo(".metas > span", 0.5, {autoAlpha: 0, y: -10}, {autoAlpha: 1, y: 0, clearProps: true, stagger: 0.1});
