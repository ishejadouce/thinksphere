...
function animateWorkScrollV($el = jQuery(document)) {
    ...
    $el.each(function () {
        ...
        gsap.set(this, {height: height, position: "relative"});


        this.classList.remove('use-v-scroll');
        const {speed = 4, start = 0, center_screen} = dataAttr(this, 'option') || {};
        ...
        jQuery(this).find('.grid-item').each(function ($index) {
            animate.to(this, 2, {yPercent: 0, y: 0, top: 0}, $index === 0 ? 0 : '-=1')
            animate.to(this, {scale: 0.95, filter: "blur(0px)"}, $index === 0 ? 0 : "-=0.9")
        });
