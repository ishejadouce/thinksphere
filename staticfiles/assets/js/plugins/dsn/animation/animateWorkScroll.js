...
function animateWorkScroll($el = jQuery(document)) {
    ...
    $el.each(function () {
        ...
        const {speed = 0, start = 0} = dataAttr(this, 'option') || {};
        ...
        animate.to(postTitle, {paddingLeft: 15}, 0);

        if (startUp)
            animate.to(startUp, {y: -150, opacity: 0}, 0);
        if (startDown)
            animate.to(startDown, {y: 150, opacity: 0}, 0);
