...
function mouseMove($element, $option) {
    ...
    const {speed = 0.35, inner = {}, ease = "power2.out", mouseStop} = $option,
        {el: elInner, speed: speedInner = 0.25} = inner;

    gsap.set([$element, elInner], {xPercent: -50, yPercent: -50});
    ...
    window.addEventListener("mousemove", e => {
        ...
        gsap.to($element, {x: e.x, y: e.y, duration: speed, ease});
        gsap.to(elInner, {x: e.x, y: e.y, duration: speedInner, ease});
    });
