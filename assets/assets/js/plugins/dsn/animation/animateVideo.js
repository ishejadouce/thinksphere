...
import {killAjax} from '../useEffect';
...
// .find('.use-horizontal-scroll-image')
    $el.each(function () {
        ...
        const {speed = 0} = dataAttr(this, 'option') || {};
        ...
        mediaMatching((context) => {
            ...
            if (bg.length && isBlur)
                animate.from(bg, {filter: "blur(5px)"}, 0);
            else {
                animate.fromTo(bg, {clipPath: "circle(5% at 50% 50%)"}, {
                    clipPath: "circle(75% at 50% 50%)"
                }, 0);

                animate.from($(this).find(".dsn-btn"), {opacity: 0.6}, 0);

            }


            if (container.length) {
                animate.to(container.find(".animate-left"), {xPercent: 50}, 0);
                animate.to(container.find(".animate-right"), {xPercent: -50}, 0);
            }
