...
function workMetroContent($el = jQuery(document)) {
    $el.find('.dsn-style-metro.dsn-metro-1 .dsn-item-post .box-content:not(.dsn-metro-content-moved,.elementor-widget-dsn_posts_slider)').each(function () {
        ...
        el.find(".post-content").css({pointerEvents: 'none'});
        el.find('.post-title').css({wordBreak: 'keep-all'});
        ...
        mediaMatching((context) => {
            ...
            el.on('mousemove', function (e) {
                ...
                if (snapLeft === 0)
                    gsap.set(elMove, {left, top, right: "auto"});
                else
                    gsap.set(elMove, {left: this.offsetWidth - elMove.offsetWidth - 10, top, right: "auto"});
            ...
            el.on('mouseleave', function (e) {
                ...
                jQuery(this).find(`${selector} .post-content`).css({pointerEvents: ''});
            });
